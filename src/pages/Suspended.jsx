...
export default function Suspended() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-(--agri-page) py-8 sm:px-6">
      {/* Brand */}
      ...
      <section className="mx-auto flex w-full max-w-2xl flex-1 items-center">
        <div className="w-full p-6 shadow-xl shadow-black/5 sm:rounded-3xl sm:border sm:border-(--agri-border-subtle) sm:bg-(--agri-card) sm:p-10">
          {/* Suspended Icon */}
          ...
          <div className="text-center">
            <h2 className="text-2xl font-bold text-(--agri-text) sm:text-3xl">
              {t("suspended.title")}
            </h2>

            <div className="mx-auto mt-5 h-1 w-20 rounded-full bg-red-500" />

            <p className="mx-auto mt-8 max-w-lg text-base leading-7 text-(--agri-text-secondary) sm:text-lg">
          ...
          <div className="mt-8 flex gap-4 rounded-2xl border border-red-500/20 bg-red-500/10 p-5">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-(--agri-card) text-red-500 shadow-sm">
              <i className="ri-information-line text-xl" />
            </div>

            <p className="leading-6 text-(--agri-text-secondary)">
          ...
          <div className="mt-8 border-t border-(--agri-border-subtle) pt-8">
            <div className="flex items-start gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[#2D6A4F]/10 text-[#2D6A4F] dark:text-(--agri-brand)">
                <i className="ri-customer-service-2-line text-2xl" />
              </div>

              <div>
                <h3 className="text-lg font-semibold text-(--agri-text)">
                  {t("suspended.needHelp")}
                </h3>

                <p className="mt-1 text-(--agri-text-secondary)">
                  {t("suspended.supportText")}
                </p>

                <span className="mt-3 inline-block font-medium text-[#2D6A4F] dark:text-(--agri-brand)">
      ...
      <footer className="mx-auto mt-10 text-center text-sm text-(--agri-text-muted)">
