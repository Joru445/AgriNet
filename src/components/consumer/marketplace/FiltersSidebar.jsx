  ...
  const content = (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-(--agri-border)">
        <h3 className="font-bold text-[#1B4332] dark:text-(--agri-brand-light) flex items-center gap-2 text-base">
            ...
            <button
              ...
              className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${
                (filters.sellingMode || "all") === option.value
                  ? "bg-[#2D6A4F]/10 border-[#2D6A4F]/40 text-[#2D6A4F] dark:bg-(--agri-brand)/10 dark:border-(--agri-brand)/40 dark:text-(--agri-brand)"
                  : "border-(--agri-border) text-(--agri-text-secondary) hover:border-gray-400 dark:hover:border-gray-500 bg-(--agri-card)"
              }`}
            >
              {option.label}
            </button>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs font-bold text-(--agri-text) flex items-center gap-1.5">
            <i className="ri-map-pin-range-line text-[#2D6A4F] dark:text-(--agri-brand)" />
      ...
      <div>
        <label className="block mb-2 text-xs font-bold text-(--agri-text) flex items-center gap-1.5">
          <i className="ri-money-dollar-circle-line text-[#2D6A4F] dark:text-(--agri-brand)" />
          {t("nearby.priceRange")}
        </label>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full border border-(--agri-border) rounded-lg pl-7 pr-3 py-2 text-sm focus:outline-none focus:border-[#2D6A4F]"
      ...
      <div>
        <label className="block mb-2 text-xs font-bold text-(--agri-text) flex items-center gap-1.5">
          <i className="ri-star-smile-line text-[#2D6A4F] dark:text-(--agri-brand)" />
          {t("nearby.minRating")}
        </label>
            ...
            <button
              ...
              className={`w-9 h-9 rounded-lg border flex items-center justify-center transition cursor-pointer ${
                filters.rating >= rating
                  ...
                  : "border-(--agri-border) text-(--agri-text-muted) hover:border-amber-300 hover:text-amber-400 bg-(--agri-card)"
              }`}
            >
      ...
      <div className="pt-4 border-t border-(--agri-border)">
        <label className="block mb-2 text-xs font-bold text-(--agri-text) flex items-center gap-1.5">
          <i className="ri-inbox-archive-line text-[#2D6A4F] dark:text-(--agri-brand)" />
          {t("nearby.availability")}
        </label>

        <label className="flex items-center justify-between p-3 rounded-xl border border-(--agri-border) bg-[#F9FBF9] dark:bg-(--agri-surface) hover:bg-[#F0F7F4] dark:hover:bg-(--agri-brand-bg) hover:border-[#2D6A4F]/40 transition-all cursor-pointer select-none">
          <div className="flex items-center gap-2.5">
            <input
              ...
              className="w-4 h-4 rounded border-(--agri-border) text-[#2D6A4F] dark:text-(--agri-brand) accent-[#2D6A4F] focus:ring-[#2D6A4F] cursor-pointer"
            />
            <div>
