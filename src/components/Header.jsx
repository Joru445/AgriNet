...
export default function Header({ user, collapsed, hideBackButton }) {
  ...
  return (
    <header className="shrink-0 sticky top-0 right-0 z-9996 flex h-[calc(4rem+env(safe-area-inset-top,0px))] pt-[env(safe-area-inset-top,0px)] items-center justify-between bg-[var(--agri-surface)]/95 border-b border-[var(--agri-border)] px-4 md:px-6 backdrop-blur-sm">
      <div className="flex items-center">
        {hideBackButton ? (
          <span
            className={`font-bold text-[var(--agri-text)] text-lg sm:text-xl lg:text-2xl whitespace-nowrap transition-all duration-300 ease-in-out ${
