...
export default function TransactionSummary({ inquiry }) {
  ...
  return (
    ...
    <section className="rounded-2xl border border-(--agri-border) bg-(--agri-card) p-5 sm:p-6 shadow-md">
      <div className="mb-4">
        <span className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-500/10 px-2.5 py-1 text-xs font-bold text-[#2D6A4F] dark:text-(--agri-brand) border border-emerald-500/20">
          <i className="ri-checkbox-circle-fill text-xs" />
          {t("transactionReview.completedTransaction")}
        </span>

        <h2 className="mt-2 text-lg font-bold text-(--agri-text)">
          {t("transactionReview.reviewTitle")}
        </h2>

        <p className="mt-0.5 text-sm text-(--agri-text-secondary) font-medium">
          {t("transactionReview.reviewSubtitle")}
        </p>
      </div>

      <div className="flex gap-4 items-center bg-(--agri-hover)/80 rounded-xl p-3.5 border border-(--agri-border-subtle) shadow-2xs">
        <img
          src={productImage}
        ...
        <div className="min-w-0 flex-1">
          <h3 className="truncate font-bold text-(--agri-text) text-base">
            {product.name || t("transactionReview.product")}
          </h3>

          {product.price != null && (
            <p className="mt-0.5 text-sm font-bold text-[#2D6A4F] dark:text-(--agri-brand)">
              ?{product.price}
          ...
          {inquiry.quantity != null && (
            <p className="mt-1 text-xs font-semibold text-(--agri-text-secondary)">
      ...
      <div className="mt-4 flex items-center gap-3 border-t border-(--agri-border-subtle) pt-4">
        <Avatar src={farmer.profilePicture} name={farmer.fullname} onClick={handleAvatarClick(farmer)} />

        <div className="min-w-0">
          <p className="text-xs font-semibold text-(--agri-text-muted)">{t("transactionReview.farmer")}</p>
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            {(farmer.verificationStatus === "approved" || farmer.verified) && (
              <span
                ...
                className="inline-flex shrink-0 items-center text-[#2D6A4F] dark:text-(--agri-brand) text-xs"
              >
