  ...
  return (
    <div className="border-b border-(--agri-border-subtle) bg-(--agri-card)/50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-3.5">
        ...
        {!loading && (
          <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-(--agri-border-subtle) bg-(--agri-card) px-3.5 py-2.5 text-xs shadow-2xs">
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-(--agri-text-secondary)">
              <span className="inline-flex items-center gap-1.5">
                ...
                <span className="font-medium text-(--agri-text-muted)">{t("farmer.secondaryRating")}:</span>
                <span className="font-bold text-(--agri-text)">{(stats.averageRating ?? 0).toFixed(1)}</span>
              </span>
              <span className="inline-flex items-center gap-1.5">
                <i className="ri-chat-smile-2-line text-purple-500 text-sm" />
                <span className="font-medium text-(--agri-text-muted)">{t("farmer.secondaryReviews")}:</span>
                <span className="font-bold text-(--agri-text)">{stats.reviewCount ?? 0}</span>
              </span>
              <span className="inline-flex items-center gap-1.5">
                <i className="ri-forbid-2-line text-rose-500 text-sm" />
                <span className="font-medium text-(--agri-text-muted)">{t("farmer.secondaryUnavailable")}:</span>
                <span className="font-bold text-(--agri-text)">{stats.unavailableProducts ?? 0}</span>
              </span>
              <span className="inline-flex items-center gap-1.5">
                <i className="ri-bookmark-line text-violet-500 text-sm" />
                <span className="font-medium text-(--agri-text-muted)">{t("farmer.secondaryReserved")}:</span>
                <span className="font-bold text-(--agri-text)">{stats.reservedInquiries ?? 0}</span>
              </span>
              <span className="inline-flex items-center gap-1.5">
                <i className="ri-mail-line text-blue-500 text-sm" />
                <span className="font-medium text-(--agri-text-muted)">{t("farmer.statUnreadMessages")}:</span>
                <span className="font-bold text-(--agri-text)">{stats.unreadMessages ?? 0}</span>
              </span>
            </div>
          </div>
        )}
