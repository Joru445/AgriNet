  ...
  return (
    <div className="flex items-center gap-1.5 mt-1.5">
      ...
      <select
        ...
        className="text-xs font-medium rounded-lg border border-(--agri-input-border) bg-(--agri-input-bg) text-(--agri-text-secondary) px-2 py-1 focus:outline-none focus:ring-1 focus:ring-[#2D6A4F] cursor-pointer"
      >
    ...
    <div className="overflow-hidden rounded-3xl border border-(--agri-border-subtle) bg-(--agri-card) shadow-sm">
      <div className="border-b border-(--agri-border-subtle) px-5 py-4 sm:px-6">
        <h2 className="flex items-center gap-2 text-base font-bold text-(--agri-text)">
          <i className="ri-store-2-line text-lg text-[#2D6A4F] dark:text-(--agri-brand)" />
      ...
      <div className="space-y-6 p-5 sm:p-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3 rounded-2xl border border-(--agri-border-subtle) bg-(--agri-elevated) px-4 py-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-(--agri-brand-bg) text-[#2D6A4F] dark:text-(--agri-brand)">
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-(--agri-border-subtle) bg-(--agri-elevated) px-4 py-3">
          ...
          {editing ? (
            <>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 rounded-xl border border-(--agri-input-border) bg-(--agri-input-bg) text-(--agri-text) placeholder-(--agri-text-muted) resize-none focus:outline-none focus:ring-2 focus:ring-[#2D6A4F]"
              />
            </>
          ) : (
            <div className="flex items-start gap-3 rounded-2xl border border-(--agri-border-subtle) bg-(--agri-elevated) px-4 py-3.5">
              <i className="ri-leaf-line mt-0.5 text-lg text-[#2D6A4F] dark:text-(--agri-brand) shrink-0" />
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-bold uppercase tracking-wider text-(--agri-text-muted)">
                  {t("profile.farmDescription")}
                </p>
                <p className="mt-0.5 text-sm leading-relaxed text-(--agri-text-secondary)">
                  {form.description || "—"}
                </p>
              </div>
            </div>
        ...
        {editing ? (
          ...
          <div className="flex items-start gap-3 rounded-2xl border border-(--agri-border-subtle) bg-(--agri-elevated) px-4 py-3.5">
            <i className="ri-map-pin-2-line mt-0.5 text-lg text-[#2D6A4F] dark:text-(--agri-brand) shrink-0" />
            <div className="min-w-0 flex-1">
              <p className="text-[11px] font-bold uppercase tracking-wider text-(--agri-text-muted)">
                {t("location.farmLocation")}
              </p>
              <p className="mt-0.5 text-sm font-medium text-(--agri-text)">
                {form.location?.address || "—"}
              </p>
            </div>
            <div className="shrink-0 mt-0.5" title={vis.address === "public" ? "Public" : "Only me"}>
              <i className={`${vis.address === "public" ? "ri-earth-line" : "ri-lock-line"} text-sm text-(--agri-text-muted)`} />
            </div>
          </div>
        )}
        ...
        {/* Statistics */}
        <div>
          <p className="mb-3 text-sm font-bold text-(--agri-text)">
            {t("profile.farmStats")}
          </p>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {statsItems.map((item) => (
              <div
                key={item.key}
                className="rounded-2xl border border-(--agri-border) bg-(--agri-elevated) p-4 text-center"
              >
                <i className={`${item.icon} text-lg text-[#2D6A4F] dark:text-(--agri-brand)`} />
                <p className="mt-1.5 text-2xl font-bold text-(--agri-text)">
                  {item.value}
                </p>
                <p className="mt-1 text-xs font-medium text-(--agri-text-muted)">
