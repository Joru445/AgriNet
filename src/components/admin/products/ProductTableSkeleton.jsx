export default function ProductTableSkeleton({ rows = 8 }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-(--agri-border-subtle) bg-(--agri-card) shadow-md shadow-black/5">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[800px]">
          <thead>
            <tr className="border-b border-(--agri-border-subtle) bg-(--agri-hover)/50">
              {Array.from({ length: 8 }).map((_, i) => (
                <th key={i} className="px-5 py-3">
                  <div className="h-4 w-20 animate-pulse rounded bg-(--agri-hover)" />
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {Array.from({ length: rows }).map((_, rowIdx) => (
              <tr key={rowIdx} className="border-b border-(--agri-border-subtle) last:border-0">
                {Array.from({ length: 8 }).map((_, colIdx) => (
                  <td key={colIdx} className="px-5 py-4">
                    <div className="h-4 animate-pulse rounded bg-(--agri-hover)" style={{ width: colIdx === 0 ? "120px" : "80px" }} />
                  </td>
                ))}
              </tr>
            ))}
          </tbody>
        </table>
      </div>
    </div>
  );
}
