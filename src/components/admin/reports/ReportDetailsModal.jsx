    ...
    <>
      ...
      >
          <div className="p-6 space-y-4">
            {/* Reason & Status Card */}
            <div className="rounded-2xl bg-(--agri-card) p-4 border border-(--agri-border-subtle) shadow-xs space-y-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-(--agri-text-muted)">
                    {t("adminReport.reportReason")}
                  </p>
                  <p className="mt-0.5 text-base font-bold text-(--agri-text)">
              ...
              {report.createdAt && (
                <div className="pt-2.5 border-t border-(--agri-border-subtle) flex items-center justify-between text-xs">
            ...
            <div className="rounded-2xl bg-(--agri-card) p-4 border border-(--agri-border-subtle) shadow-xs space-y-1.5">
              <p className="text-xs font-bold text-(--agri-text) uppercase tracking-wide">
                {t("adminReport.descriptionTitle")}
              </p>
              <div className="rounded-xl bg-(--agri-hover)/80 border border-(--agri-border-subtle) p-3.5 text-xs sm:text-sm leading-relaxed text-(--agri-text) font-medium whitespace-pre-wrap">
            ...
            {report.evidenceUrl && (
              <div className="rounded-2xl bg-(--agri-card) p-4 border border-(--agri-border-subtle) shadow-xs space-y-2">
                <p className="text-xs font-bold text-(--agri-text) uppercase tracking-wide">
                  {t("adminReport.attachedEvidence")}
                </p>
                <div
                  onClick={() => setShowEvidenceViewer(true)}
                  className="relative group inline-block rounded-2xl overflow-hidden border border-(--agri-border) bg-black/5 shadow-xs cursor-pointer"
                >
                  <img
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-4 shadow-xs flex flex-col justify-between space-y-3">
                <div>
                  ...
                  <p className="text-sm font-bold text-(--agri-text) mt-1.5">
                  ...
                  {reportedUser?.role && (
                    <span className="inline-block mt-1.5 rounded-md bg-(--agri-hover) px-2 py-0.5 text-[10px] font-bold text-(--agri-text-secondary) capitalize border border-(--agri-border)">
                      {reportedUser.role}
                    </span>
                  )}
                </div>
              ...
              <div className="rounded-2xl border border-(--agri-border-subtle) bg-(--agri-card) p-4 shadow-xs flex flex-col justify-between space-y-3">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider text-(--agri-text-muted)">
            ...
            {targetType === "product" && targetProduct && (
              <div className="rounded-2xl bg-(--agri-card) p-4 border border-(--agri-border-subtle) shadow-xs space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-(--agri-text-muted)">{t("adminReport.productName")}:</span>
                    <span className="ml-1 font-bold text-(--agri-text)">{targetProduct.title || "—"}</span>
                  </div>
                  <div>
                    <span className="text-(--agri-text-muted)">{t("adminReport.category")}:</span>
                    <span className="ml-1 font-bold text-(--agri-text)">{targetProduct.category || "—"}</span>
                  </div>
                  <div>
                    <span className="text-(--agri-text-muted)">{t("adminReport.sellingMode")}:</span>
                    <span className="ml-1 font-bold text-(--agri-text) capitalize">{targetProduct.sellingMode || "—"}</span>
                  </div>
                  <div>
                    <span className="text-(--agri-text-muted)">{t("adminReport.availability")}:</span>
                  ...
                  {targetProduct.sellingMode === "preorder" && (
                    <>
                      <div>
                        <span className="text-(--agri-text-muted)">{t("adminReport.preOrderLimit")}:</span>
                        <span className="ml-1 font-bold text-(--agri-text)">{targetProduct.preOrderLimit ?? "—"}</span>
                      </div>
                      <div>
                        <span className="text-(--agri-text-muted)">{t("adminReport.reserved")}:</span>
                        <span className="ml-1 font-bold text-(--agri-text)">{targetProduct.reservedQuantity ?? 0}</span>
                      </div>
                    </>
                  )}
                </div>
            ...
            <div className="rounded-2xl bg-(--agri-card) p-4 border border-(--agri-border-subtle) shadow-xs space-y-1.5">
              <label htmlFor="admin-resolution-notes" className="block text-xs font-bold text-(--agri-text) uppercase tracking-wide">
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-(--agri-border) bg-(--agri-hover)/50 p-3 text-xs sm:text-sm text-(--agri-text) font-medium placeholder-(--agri-text-muted) focus:bg-(--agri-card) focus:border-[#2D6A4F] focus:ring-2 focus:ring-[#2D6A4F]/20 focus:outline-none transition resize-none"
              />
            </div>
