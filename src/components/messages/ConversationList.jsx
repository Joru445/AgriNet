      ...
      <div className="p-4">
        ...
        <div className="relative flex items-center gap-2 bg-(--agri-card) rounded-xl px-3 py-1.5 border-2 border-(--agri-border) shadow-xs focus-within:border-agri-primary focus-within:shadow-md focus-within:ring-3 focus-within:ring-[#2D6A4F]/15 transition-all">
          ...
          <input
            ...
            className="w-full text-sm font-semibold text-(--agri-text) placeholder-(--agri-text-muted) focus:outline-none bg-transparent"
          />
          ...
          {search && (
            <button
              type="button"
              onClick={() => onSearch("")}
              className="p-0.5 rounded-full text-(--agri-text-muted) hover:text-(--agri-text-secondary) hover:bg-(--agri-hover) transition cursor-pointer shrink-0"
        ...
        {!searching ? (
          <>
            <div className="px-4 pt-4 pb-2">
              <p className="text-xs uppercase tracking-wide text-(--agri-text-muted) font-semibold">
            ...
            {loading ? (
              <div className="space-y-1 p-2 animate-pulse">
                {Array.from({ length: 6 }).map((_, i) => (
                  ...
                  >
                    <div className="w-11 h-11 rounded-full bg-(--agri-hover) shrink-0" />
                    <div className="flex-1 space-y-1.5 min-w-0">
                      <div className="h-3.5 w-24 bg-(--agri-hover) rounded" />
                      <div className="h-2.5 w-36 bg-(--agri-hover) rounded" />
                    </div>
                  </div>
                ))}
              </div>
            ) : conversations.length === 0 ? (
              <div className="text-center text-(--agri-text-muted) py-10 px-6">
          ...
          <>
            ...
            {conversations.length === 0 && users.length === 0 && (
              <div className="text-center py-10 px-6 text-(--agri-text-muted)">
